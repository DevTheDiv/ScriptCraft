...
***/
// prettier-ignore
// (required so a comma won't be added to last line)
...
***/
  function resolveModuleToFile(moduleName, parentDir) {
    ...
    if (moduleName.match(/^[^\.\/]/)) {
      // it's a module named like so ... './lib/events.js' , 'net/http'
      //
  ...
  function _require(parentFile, path, options) {
    ...
    canonizedFilename = file.getCanonicalPath().replaceAll('\\', '/');
    ...
    moduleInfo = {
      ...
      exports: {},
      require: _requireClosure(file.getParentFile())
    };
    ...
    try {
      ...
      /*
       ...
       This can be fixed by instead using eval
       */
      throw new Error(
    ...
    var __dirname = '' + file.getParentFile().getCanonicalPath();
    var parameters = [
