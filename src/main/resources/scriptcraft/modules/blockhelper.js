'use strict';
/*global module, exports, require, Packages, __plugin__, server*/
...
function applyFacing(block, metadata) {
  ...
  if (blocks.isStair(block.typeId)) {
    ...
    switch (block.typeId) {
      ...
      case blocks.torch:
        face(
          ['up' /* default */, 'east', 'west', 'south', 'north', 'up'][metadata]
        );
        break;
    }
  }
  ...
  switch (block.typeId) {
    case blocks.sign_post:
      if (metadata !== 0) {
        property(block).set(
          'rotation',
          new Packages.java.lang.Integer(metadata)
        );
      }
  }
}
function applyVariant(block, metadata) {
  var cmQuartzProperties =
    Packages.net.canarymod.api.world.blocks.properties.helpers.QuartzProperties;
  switch (block.typeId) {
    case blocks.quartz:
      cmQuartzProperties.applyVariant(
        block,
        cmQuartzProperties.Variant.valueOf(metadata)
      );
      break;
  }
}
